/**
 * parallel_tsp_qe_serialization.cpp
 * QE load-balanced parallel B&B using explicit MPI_Pack/MPI_Unpack for BBNode.
 * More portable but higher overhead than direct struct send.
 * Usage: mpirun -np <N> ./parallel_tsp_qe_serialization [input_file]
 */
...
static float neighbor_thresholds[2];
...
static void send_work(int requester_rank) {
    ...
    for (auto* nd : to_send) {
        pack_bbnode(nd, buf, PACK_BUF_SIZE);
        MPI_Send(buf, PACK_BUF_SIZE, MPI_CHAR, requester_rank, TAG_QE_WORK_TRANSFER, MPI_COMM_WORLD);
        delete nd;
    }
}
    ...
    while (flag) {
        ...
        if (tag == TAG_QE_THRESHOLD) {
            ...
            for (int i = 0; i < count; i++) {
                MPI_Recv(buf, PACK_BUF_SIZE, MPI_CHAR, src, TAG_QE_WORK_TRANSFER, MPI_COMM_WORLD, &status);
                BBNode* nd = unpack_bbnode(buf, PACK_BUF_SIZE);
...
int main(int argc, char* argv[]) {
    ...
    if (my_rank == 0) {
        std::cout << "=== Parallel TSP QE (Serialization) | " << psize << " procs ===" << std::endl;
        ...
        for (int i = 0; i < (int)subs.size(); i++) {
            ...
            else { pack_bbnode(subs[i], buf, PACK_BUF_SIZE); MPI_Send(buf, PACK_BUF_SIZE, MPI_CHAR, dest, TAG_INITIAL_WORK, MPI_COMM_WORLD); delete subs[i]; }
        }
        ...
        char buf[PACK_BUF_SIZE]; MPI_Status st;
        MPI_Recv(buf, PACK_BUF_SIZE, MPI_CHAR, 0, TAG_INITIAL_WORK, MPI_COMM_WORLD, &st);
        int cnt; MPI_Get_count(&st, MPI_CHAR, &cnt);
        if (cnt > 0) { local_pq.push(unpack_bbnode(buf, PACK_BUF_SIZE)); current_state = ACTIVE; }
    }
    ...
    if (my_rank == 0) {
        ...
        std::cout << "Optimal cost:   " << final_gub << "\nExecution time: " << exec_time << " s\nTotal nodes:    " << total << "\nStd dev:        " << sqrt(var/psize) << std::endl;
    }
