/**
 * parallel_tsp_qe_weighted_full.cpp
 * 
 * Advanced QE load-balanced parallel B&B with a weighted progress heuristic.
 * Key innovation: weighted_heuristic_value() factors in tour progress for
 * more nuanced load assessment during QE balancing.
 * 
 * W(N) = LB(N) + LB(N) * WEIGHT_PROGRESS_PENALTY * progress_ratio(N)
 * 
 * Usage: mpirun -np <N> ./parallel_tsp_qe_weighted_full [input_file]
 */
...
static int neighbor_ranks[2];
static float neighbor_thresholds[2];    // Now stores WEIGHTED thresholds
static bool requested_work_from[2];
...
// ---------- WEIGHTED QE: threshold uses weighted_heuristic_value ----------
static void send_threshold_cost() {
    float threshold = INF;
    int n = city_graph_ptr->n_city;
    ...
    if (!local_pq.empty()) {
        ...
        if (!temp.empty()) {
            // USE WEIGHTED heuristic for threshold
            threshold = temp.back()->weighted_heuristic_value(n);
        }
...
static void request_work(int neighbor_rank) {
    int r = my_rank;
    MPI_Send(&r, 1, MPI_INT, neighbor_rank, TAG_QE_WORK_REQUEST, MPI_COMM_WORLD);
}
...
// ---------- WEIGHTED QE: compare using weighted heuristic ----------
static void perform_load_balancing_check() {
    send_threshold_cost();
    int n = city_graph_ptr->n_city;

    // Use WEIGHTED value for self-assessment
    float my_best_weighted = local_pq.empty() ? INF :
        local_pq.top()->weighted_heuristic_value(n);

    for (int i = 0; i < 2; i++) {
        if ((local_pq.empty() || my_best_weighted > neighbor_thresholds[i])
            && !requested_work_from[i]) {
...
static void send_termination_signal() {
    current_state = TERM_SENT; local_termination_signal_sent = true;
    for (int r = 0; r < psize; r++) {
        if (r != my_rank) { int d = my_rank; MPI_Send(&d, 1, MPI_INT, r, TAG_TERM_CHECK, MPI_COMM_WORLD); }
    }
    if (my_rank == 0) terminated_count++;
}

static void send_wakeup_signal() {
    local_termination_signal_sent = false; current_state = ACTIVE;
    for (int r = 0; r < psize; r++) {
        if (r != my_rank) { int d = my_rank; MPI_Send(&d, 1, MPI_INT, r, TAG_TERM_WAKEUP, MPI_COMM_WORLD); }
    }
    for (int i = 0; i < 2; i++) {
        if (!received_term_confirm[i]) {
            int d = my_rank; MPI_Send(&d, 1, MPI_INT, neighbor_ranks[i], TAG_TERM_CONFIRM, MPI_COMM_WORLD);
...
static void check_termination_condition() {
    if (current_state != IDLE || !local_pq.empty() || local_termination_signal_sent) return;
    // NOTE: Compares weighted neighbor thresholds with unweighted GUB
    // This is a known design point discussed in the report
    bool neighbors_done = true;
    for (int k = 0; k < 2; k++)
        if (neighbor_thresholds[k] < global_upper_bound) { neighbors_done = false; break; }
    if (!neighbors_done || !received_term_confirm[0] || !received_term_confirm[1]) return;
    send_termination_signal();
}

static void handle_incoming_messages() {
    int flag = 1; MPI_Status status;
    while (flag) {
        ...
        if (tag == TAG_QE_THRESHOLD) {
            float thresh; MPI_Recv(&thresh, 1, MPI_FLOAT, src, tag, MPI_COMM_WORLD, &status);
            int idx = (src == neighbor_ranks[0]) ? 0 : 1;
            neighbor_thresholds[idx] = thresh; requested_work_from[idx] = false;
        } else if (tag == TAG_QE_WORK_REQUEST) {
            int req; MPI_Recv(&req, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);
            send_work(req);
        } else if (tag == TAG_QE_WORK_TRANSFER) {
            int count; MPI_Recv(&count, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);
            for (int i = 0; i < count; i++) {
                BBNode* nd = new BBNode();
                MPI_Recv(nd, sizeof(BBNode), MPI_BYTE, src, TAG_QE_WORK_TRANSFER, MPI_COMM_WORLD, &status);
                if (nd->estimated_cost() < global_upper_bound) local_pq.push(nd);
                else delete nd;
            }
            if (count > 0 && !local_pq.empty() && (current_state == IDLE || current_state == TERM_SENT))
                send_wakeup_signal();
        } else if (tag == TAG_TERM_CHECK) {
            int d; MPI_Recv(&d, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);
            if (my_rank == 0) terminated_count++;
        } else if (tag == TAG_TERM_WAKEUP) {
            int d; MPI_Recv(&d, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);
            if (my_rank == 0 && terminated_count > 0) terminated_count--;
            if (local_termination_signal_sent) { local_termination_signal_sent = false; if (current_state == TERM_SENT) current_state = IDLE; }
        } else if (tag == TAG_TERM_CONFIRM) {
            int d; MPI_Recv(&d, 1, MPI_INT, src, tag, MPI_COMM_WORLD, &status);
            int idx = (src == neighbor_ranks[0]) ? 0 : 1; received_term_confirm[idx] = true;
        } else {
            char junk[4096]; MPI_Recv(junk, 4096, MPI_BYTE, src, tag, MPI_COMM_WORLD, &status);
        }
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (my_rank == 0) { if (!parse_tsp_input(input_file, &city_graph)) MPI_Abort(MPI_COMM_WORLD, 1); }
    ...
    global_upper_bound = INF; nodes_processed = 0;
    current_state = IDLE; local_termination_signal_sent = false;
    terminated_count = 0; globally_terminated = false;
    ...
    if (my_rank == 0) {
        std::cout << "=== Parallel TSP QE-Weighted-Full | " << psize << " procs | penalty="
                  << WEIGHT_PROGRESS_PENALTY << " ===" << std::endl;
        PriorityQueue init_pq;
        BBNode* root = create_root_node(&city_graph); init_pq.push(root);
        std::vector<BBNode*> subs; int target = psize * 2;
        while (!init_pq.empty() && (int)subs.size() < target) {
            BBNode* nd = init_pq.top(); init_pq.pop(); nodes_processed++;
            if (nd->is_solution(n)) { float c = nd->hamilton_cost(&city_graph); if (c < global_upper_bound) global_upper_bound = c; delete nd; continue; }
            for (int c = 0; c < n; c++) { if (!nd->visited[c]) { BBNode* ch = nd->visit_city(c, &city_graph); if (ch) subs.push_back(ch); } }
            delete nd;
        }
        ...
        for (int r = 1; r < psize; r++) {
            bool got = false; for (int i = r; i < (int)subs.size(); i += psize) { got = true; break; }
        ...
        if (cnt >= (int)sizeof(BBNode)) { BBNode* nd = new BBNode(tmp); local_pq.push(nd); current_state = ACTIVE; }
    }
    ...
    while (!globally_terminated) {
        ...
        if (!local_pq.empty()) {
            ...
            else if (node->is_solution(n)) { float cost = node->hamilton_cost(&city_graph); if (cost < global_upper_bound) global_upper_bound = cost; delete node; }
            else {
                for (int c = 0; c < n; c++) {
                    if (!node->visited[c]) { BBNode* child = node->visit_city(c, &city_graph); if (child && child->estimated_cost() < global_upper_bound) local_pq.push(child); else if (child) delete child; }
                }
                delete node;
            }
        } else { current_state = (current_state == TERM_SENT) ? TERM_SENT : IDLE; }

        iter++;
        if (iter % GUB_UPDATE_INTERVAL == 0) { float lg = global_upper_bound; MPI_Allreduce(&lg, &global_upper_bound, 1, MPI_FLOAT, MPI_MIN, MPI_COMM_WORLD); }
        if (iter % LB_CHECK_INTERVAL == 0) perform_load_balancing_check();
        if (current_state == IDLE) check_termination_condition();
        if (my_rank == 0 && terminated_count >= psize && local_pq.empty()) globally_terminated = true;
    ...
    if (my_rank == 0) {
        ...
        for (int i = 0; i < psize; i++) { total += all_nodes[i]; if (all_nodes[i] > max_n) max_n = all_nodes[i]; if (all_nodes[i] < min_n) min_n = all_nodes[i]; }
        double avg = (double)total / psize, var = 0;
        ...
        std::cout << "Avg nodes:      " << avg << std::endl;
